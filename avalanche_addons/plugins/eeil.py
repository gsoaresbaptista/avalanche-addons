...
class EEIL(SupervisedPlugin):
    ...
    def __init__(self, mem_size, stage_2_epochs=12, noise_grad=True, clipgrad=10000):
    ...
    def before_backward(self, strategy, **kwargs):
        ...
        if task_id != 0:
            ...
            for i in range(task_id + 1):
                outputs.append(strategy.model(strategy.mb_x.to(strategy.device), i))
                if i != task_id:
                    outputs_old.append(self.model_old(strategy.mb_x.to(strategy.device), i))

            return self.cross_distilled_loss(outputs, strategy.mb_y, outputs_old)
        else:
    ...
    def after_backward(self, strategy, **kwargs):
        ...
        if self.noise_grad:
            self._noise_grad(strategy.model.parameters(), strategy.clock.train_iterations)
        ...
        if task_id != 0:
            ...
            for e in range(self.stage_2_epochs):
                ...
                for inputs in loader:
                    ...
                    loss = self.cross_distilled_loss(outputs, y_real, outputs_old)
    ...
    def cross_distilled_loss(self, outputs, targets, outputs_old = None):
        ...
        if outputs_old:
            for output, output_old in zip(outputs, outputs_old):
                loss += self.lamb * F.binary_cross_entropy(F.softmax(output / self.T, dim=1),
